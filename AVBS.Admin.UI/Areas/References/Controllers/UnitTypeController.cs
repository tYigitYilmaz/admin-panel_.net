...
namespace AVBS.Admin.UI.Areas.References.Controllers {
    [Authentication]
    public class UnitTypeController : IController< AVBS_UnitType, BaseBusiness<AVBS_UnitType>> {
        ...
        public override void ModelOperations ( ref AVBS_UnitType model ) {
        ...
        public JsonResult GetList (  JQueryDataTableParam param, int locationid = 0 ) {
            ...
            try {
                var cashBookBusiness = new BaseBusiness<AVBS_UnitType>();
                ...
                // Configure Order
                var orderingFunction = new Func<AVBS_UnitType, object>( c => c.Id );
                if ( orderColumn == 0 ) {
