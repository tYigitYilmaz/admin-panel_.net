...
namespace AVBS.Admin.UI.Areas.References.Controllers {
    [Authentication]
    public class WorkWithCompanyController : IController<AVBS_WorkWithCompany, BaseBusiness<AVBS_WorkWithCompany>> {
        ...
        public override void ModelOperations ( ref AVBS_WorkWithCompany model ) {
        ...
        public JsonResult GetList (  JQueryDataTableParam param, int locationid = 0 ) {
            ...
            try {
                var cashBookBusiness = new BaseBusiness<AVBS_WorkWithCompany>();
                ...
                // Configure Order
                var orderingFunction = new Func<AVBS_WorkWithCompany, object>( c => c.Id );
                if ( orderColumn == 0 ) {
