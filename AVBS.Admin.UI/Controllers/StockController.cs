...
namespace AVBS.Admin.UI.Controllers {
    [Authentication]
    public class StockController : IController< AVBS_Stock, BaseBusiness<AVBS_Stock>> {
        ...
        public override void ModelOperations ( ref AVBS_Stock model ) {
        ...
        public JsonResult GetList (  JQueryDataTableParam param, string date ) {
            ...
            try {
                var cashBookBusiness = new ViewBusiness<AVBS_StockTableView>();
                ...
                // Configure Order
                var orderingFunction = new Func<AVBS_StockTableView, object>( c => c.Id );
                if ( orderColumn == 0 ) {
